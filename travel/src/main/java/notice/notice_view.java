...
public class notice_view extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//조회수 +1증가 
		m_notice_update vc = new m_notice_update();
		vc.viewcount(Integer.parseInt(nidx));
		
		//공지 상세페이지 보기
		m_notice_select ns = new m_notice_select(Integer.parseInt(nidx));
		ArrayList<String> n_view = ns.notice_onedata(Integer.parseInt(nidx));
