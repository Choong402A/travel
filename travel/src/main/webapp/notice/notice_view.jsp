<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>공지사항 내용 확인 페이지</title>
    <link rel="stylesheet" type="text/css" href="../admin/css/basic.css?v=2">
    <link rel="stylesheet" type="text/css" href="../admin/css/login.css?v=11">
    <link rel="stylesheet" type="text/css" href="../admin/css/main.css?v=11">
    <link rel="stylesheet" type="text/css" href="../admin/css/notice.css?v=11">
    <link rel="icon" href="../admin/img/logo.png" sizes="128x128">
    <link rel="icon" href="../admin/img/logo.png" sizes="64x64">
    <link rel="icon" href="../admin/img/logo.png" sizes="32x32">
    <link rel="icon" href="../admin/img/logo.png" sizes="16x16">
</head>
<body>
	<!-- 헤더부분 -->
	<%@ include file="./top.jsp" %>
	<!-- 헤더부분 -->
	
	<!-- 메인부분 -->
	<%@ include file="./notice_view_detail.jsp" %>
	<!-- 메인부분 -->
	
	<!-- 푸터부분 -->
	<%@ include file="./footer.jsp" %>
	<!-- 푸터부분 -->
</body>
</html>